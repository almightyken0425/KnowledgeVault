...
> [!SUMMARY] 核心差異概覽
> - **iOS / Android 原生 App**：各自是一個**單一、直接**的執行體系。程式碼都是對應平台的「母語」，指令可以直接下達給作業系統。
> - **React Native App**：是一個**雙軌、協同**的執行體系，由 **JavaScript 世界**和**原生世界**同時運作，並透過一座「橋樑 (Bridge)」進行溝通。
...
|**原生功能**|**直接呼叫原生框架**： 程式碼直接 `import` 並呼叫 Apple 提供的原生框架 (例如 `AVFoundation` 存取相機)。|**直接呼叫原生函式庫**： 程式碼直接 `import` 並呼叫 Google 提供的原生函式庫 (例如 `android.hardware.camera2` 存取相機)。|**透過「橋樑」請求**： JS 程式碼發送一個「打開相機」的請求**跨越 Bridge**。原生端的模組收到後，在 iOS 上呼叫 `AVFoundation`，在 Android 上呼叫 `android.hardware.camera2`。|
...
在軟體開發中，「Runtime」這個詞主要有兩種緊密相關但必須區分的含義：
- Runtime 運行階段
...
- **指令被執行**：CPU 開始讀取（或已被 `[[編譯器]]` 翻譯好的）指令並執行計算。
...
如果說 **Runtime** 是舞台上正在上演的**話劇本身**，那麼 **Runtime Environment** 就是讓這齣話劇得以演出的**整個劇院**（包含舞台、燈光、音響和後台工作人員）。
...
- API 是執行環境的「手腳」，它是一系列由環境預先提供的**指令與工具集合**，讓程式碼可以與外部世界互動。
...
- 伺服器 Application 通常是「無頭 (Headless)」的，沒有自己的使用者介面。
...
- **核心特徵**：
    ...
    - **高度可擴充**：透過安裝「擴充套件 (Extensions)」，可以增加對不同語言的支援和各種輔助功能。
...
>     
> - **擴充套件** 則像是您為這個工作檯添購的各式**專用工具配件**（鑽頭、量尺、夾具），讓它能勝任更專業、更複雜的工作。
>     
...
- **普遍定義**：一個能讓特定程式語言的程式碼成功運行的「完整支援系統」。
...
- **第一部分：核心名詞解析 (The What) - 認識我們的工具箱**
    ...
    - **1. 基礎概念 (Fundamental Concepts)**
        ...
        - **整合開發環境 (IDE) vs. 程式碼編輯器 (Code Editor)**
            ...
            - IDE：重量級、功能全面、內建編譯器 (Compiler)。(例如：Xcode)
    ...
    - **2. 開發環境的基石 (The Foundation)**
        ...
        - **原生整合開發環境 (Native IDEs)**
            ...
            - **Xcode**
                ...
                - 角色定位：Apple 官方的 iOS/macOS App 製造工廠。
                    
                - 核心組成：iOS SDK、編譯器、模擬器。
                    
            - **Android Studio**
                
                - 角色定位：Google 官方的 Android App 製造工廠。
                    
                - 核心組成：Android SDK、Gradle 建置系統、模擬器。
    ...
    - **3. App 的構成零件 (The Building Blocks)**
        ...
        - **專案套件/函式庫 (Packages/Libraries)**
            ...
            - 與 App 的關係：會被打包進最終的 App 檔案中。
                
        - **`node_modules`**
            
            - 定義：存放所有已下載專案套件的本地資料夾。
    ...
    - **4. 核心框架與工具 (The Core Frameworks & Tools)**
        ...
        - **React Native**
            ...
            - 核心機制：透過「橋樑 (Bridge)」讓 JavaScript 控制原生 UI。
