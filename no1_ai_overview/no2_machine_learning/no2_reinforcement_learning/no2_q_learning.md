...
- **真實獎勵:** 這一布走下去，環境立刻給你的分數 R。
- **對未來的預期:** 假設下一布我也選這最好的走法，未來最多還能拿幾分？
- **折扣因子 Gamma:** 未來的分數要打折。今天的 100 元比明年的 100 元值錢。

## 訓練流程

- **初始化:** 建立一個 Q-Table，所有分數通常設為 0。
- **迴圈 Loop:**
    - **探索策略:** 決定要亂走還是查表。通常使用 Epsilon-Greedy 策略，即 90% 查表，10% 亂走以探索新路徑。
    - **執行:** 做出動作，獲得 Reward，進入下一個 State。
    - **更新:** 套用公式更新 Q-Table 中剛才那一格的分數。
- **收斂:** 重複千萬次後，Q-Table 的分數會固定下來，這就是 Agent 學到的 最佳策略 Optimal Policy。
