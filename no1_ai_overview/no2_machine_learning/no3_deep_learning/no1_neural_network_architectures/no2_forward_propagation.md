...
- **第一步：加權 Weighing**
    ...
    - **意義:** 判斷這個訊號有多重要。
    - **資料結構:** 這個神經元的權重是一個向量，例如 `[w1, w2, w3]`，這就是 **神經元節點 Neuron Node** 在 overview 中定義的型態。
    - *例子*：如果這個神經元負責找車輪，那看到圓形的訊號權重就會很高。
- **第二步：加總 Summation**
    - **動作:** 把所有乘完權重的訊號加起來。
    - **意義:** 綜合評估所有線索。
    - **程式實現:** 這就是 `processLayer` 函數中的矩陣運算核心：$Z = X \cdot W$。
- **第三步：偏差修正 Adding Bias**
    - **動作:** 加上一個數值 **偏差** $b$。
    - **意義:** 調整門檻。
    - **資料結構:** 每個 **Layer** 都有自己的 `biases` 陣列，每個神經元分配一個偏差值。
    - *例子*：雖然總分只有 50 分，但如果 Bias 是 +10，總分就變成 60 分，及格。
- **第四步：激活 Activation**
    - **動作:** 通過 **激活函數 如 ReLU**。
    - **意義:** 決定是否放行。
    - **程式實現:** 調用 `activation(result)` 函數，通常是 ReLU，將負值歸零。
    - *例子*：如果算出來是負數，ReLU 會把它變成 0，這個特徵不存在，不要傳給下一站。

**整體流程：從 Layer 到輸出**

當這四個步驟在整個 **Layer** 中執行時：
- **權重矩陣 Weights Matrix** 包含了所有神經元的權重向量，形狀為 `[N, M]`，N 是神經元數量。
- **並聯運算:** 所有神經元同時處理同一筆輸入，產生 N 個輸出值。
- **接力傳遞:** 這一層的輸出成為下一層的輸入，在 **Neural Network Model** 中層層傳遞。
...
- **數據結構:**
    ...
    - **Weights** $W$: 一個巨大的矩陣。
- **運算邏輯:**
    - 原本的單一神經元運算 `sum(x * w) + b` 在程式中變成了：$$Z = X \cdot W + B$$
    - 這就是 `processLayer(inputs, weights, biases)` 函數的本質。
    - 一行程式碼就能解決，例如 Python numpy 中的 `np.dot(X, W) + B`。
