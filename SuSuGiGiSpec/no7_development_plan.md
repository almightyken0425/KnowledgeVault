...
- [ ] 實作 App 啟動時的定期交易產生邏輯 (`2. 核心功能規格.md` 2.7 節)。
    ...
    - [ ] 實作交易列表點擊後的「僅此一筆 / 未來所有」編輯/刪除邏輯 (於 `5.3`/`5.4` 中)。
    
    - [ ] 實作 `SearchScreen.tsx` (搜尋 `Note` 欄位) (根據 `5.14` 規格)。
    
    - [ ] 實作 `ImportScreen.tsx` (CSV 匯入交易邏輯) (根據 `5.15` 規格)。
    
    - [ ] 實作**付費牆 (PaywallScreen)** (根據 `5.16` 規格)。
