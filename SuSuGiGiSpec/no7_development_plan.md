...
## 1. Phase 0: 基礎建設 (Setup)

- [ ] 建立 Expo (React Native) 專案。
    
- [ ] 實作 `src/types/index.ts` (根據 `1. 資料結構定義.md`)。
...
- [ ] 建立 `LoginScreen.tsx` (UI + Google 登入邏輯) (根據 `5.1` 規格)。
...
## 3. Phase 2: 核心資料管理 (Data Management)
...
- [ ] 建立 `CurrencyRateScreen.tsx` (手動輸入匯率介面) (根據 `5.8` 規格)。
    
- [ ] 實作 `PreferenceScreen.tsx` (偏好設定 UI - 時區/基礎貨幣/語系) (根據 `5.9` 規格)。
    
- [ ] 實作 `SettingsScreen.tsx` (主頁及登出按鈕) (根據 `5.5` 規格)。
...
- [ ] 建立 `TransferEditorScreen.tsx` (同幣別 + 跨幣別轉帳 UI，串接 `firestoreService`) (根據 `5.4` 規格)。
...
## 6. Phase 5: MVP 進階功能 (MVP Features)

- [ ] 實作 **`定期交易 (Schedules)`** 建立介面 (Modal/Sheet，於 `5.3`/`5.4` 中) (付費功能)。
    
- [ ] 實作 App 啟動時的定期交易產生邏輯 (`2. 核心功能規格.md` 2.7 節)。
    
- [ ] 實作交易列表點擊後的「僅此一筆 / 未來所有」編輯/刪除邏輯 (於 `5.2`/`5.10` 中)。
    
- [ ] 實作 `SearchScreen.tsx` (搜尋 `Note` 欄位) (根據 `5.10` 規格)。
    
- [ ] 實作 `ImportScreen.tsx` (CSV 匯入交易邏輯) (根據 `5.11` 規格)。
    
- [ ] 實作**付費牆 (PaywallScreen)** (根據 `5.12` 規格)。
...
## 7. Phase 6: 測試與打包 (Testing & Release)
