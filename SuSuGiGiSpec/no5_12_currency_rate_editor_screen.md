...
- **2.2. 貨幣對顯示區 (Currency Pair Display):**
    ...
    - **邏輯:**
        - 在「更新」模式下，此區域會顯示被點擊的貨幣對。
        - 在「新增」模式下，此區域可能需要讓使用者選擇貨幣。
...
- **3.1. 模式判斷 (Mode Detection):**
    - 畫面載入時，檢查傳入的參數。如果傳入了 `rateId`，則進入「更新」模式；否則，進入「新增」模式。

- **3.2. 資料載入邏輯 (更新模式):**
    - 使用傳入的 `rateId`，從 `CurrencyRates` 表中讀取對應的匯率資料，並填充到 UI 元件中。

- **3.3. 儲存邏輯:**
    - 點擊「儲存」按鈕時，執行以下步驟：
        1.  **資料驗證:** 檢查匯率是否為有效數字且大於 0。
        2.  **儲存資料 (一律新增):**
            - 無論是「新增」或「更新」模式，此畫面的儲存操作**一律**為**建立一筆新**的 `CurrencyRates` 記錄。這遵循了「只增不改」的原則，以確保資料同步的穩健性並保留歷史紀錄。
        3.  **返回:** 儲存成功後，自動返回 `CurrencyRateScreen`，並觸發該畫面的資料重新整理。

- **3.4. 刪除邏輯:**
    - 點擊「刪除」按鈕並確認後，從 `CurrencyRates` 表中刪除該筆記錄，然後返回 `CurrencyRateScreen`。
...
- 使用 `useState` 管理以下狀態：
    ...
    - `mode: 'add' | 'update'`：當前畫面的模式。

## 5. 導航 (Navigation)

- **進入:**
    - 從 `CurrencyRateScreen` (5.11) 的「新增」按鈕點擊進入（新增模式）。
    - 從 `CurrencyRateScreen` (5.11) 的列表項目點擊進入（更新模式）。
- **退出:**
