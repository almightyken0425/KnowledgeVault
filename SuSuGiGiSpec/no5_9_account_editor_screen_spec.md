# 5.9. 帳戶編輯器畫面 (AccountEditorScreen)
...
- **3.2. 儲存邏輯 (Save Logic):**
    - **新增模式:**
        - **(付費牆檢查)** 檢查目前使用者帳戶數量是否已達免費版上限 (3個)。若已達上限，則導航至 `PaywallScreen` (`5.16`)。
        - **(付費牆檢查 - 多幣別)** 若使用者選擇了非基礎貨幣，檢查 `isPremiumUser` 狀態。若為免費版，導航至 `PaywallScreen` (`5.16`)。
...
- **3.3. 刪除邏輯 (Delete Logic):**
    ...
    - 刪除成功後，關閉畫面並導航返回 `AccountListScreen`。
...
- **進入:** 從 `AccountListScreen` (5.8) 的「新增」按鈕或列表項目點擊進入。
