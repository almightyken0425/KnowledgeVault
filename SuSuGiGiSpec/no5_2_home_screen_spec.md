...
- **逐筆紀錄區塊 (`FlatList`):**
    ...
    - **顯示內容 (每筆交易):**
    - **未來排程提醒:** 依 `no0_pending_discussion.md` 要求，可在當期清單最上方顯示即將發生的定期交易，並以顏色/貼標區隔，但僅供提醒，不允許切換到今天以後的期間。
...
- **時間區間切換 (左右滑動手勢):**
    ...
    - **資料更新:** 允許的滑動操作會重新計算**圓餅圖區塊**、**總餘額與控制區塊**以及**逐筆紀錄區塊**的數據並觸發動畫。
