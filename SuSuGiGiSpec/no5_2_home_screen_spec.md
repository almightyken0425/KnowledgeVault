...
- **2.3 逐筆紀錄區塊 (FlatList):**
    ...
    - **顯示模式 (可切換):** 根據 `2.2` 中切換按鈕的狀態顯示：
        ...
        - **模式 B (按類別分組):** 將交易按 `CategoryId` 匯總，顯示各類別總金額，並可點擊展開查看該類別下的交易明細。
            
    - **顯示內容 (每筆交易):**
    - **未來排程提醒:** 依 `no0_pending_discussion.md` (7-14) 要求，可在當期清單最上方顯示即將發生的定期交易，並以顏色/貼標區隔，但僅供提醒，不允許切換到今天以後的期間。
...
- **3.2 時間區間切換 (左右滑動手勢):**
    ...
    - **行為:** 僅允許由當前期間向左（查看過去）滑動；嘗試往右進入今天以後的期間時需鎖定頁面並以 Toast/Banner 提示「不可瀏覽未來」，遵循 `no0_pending_discussion.md` (7-14)。
        
    - **資料更新:** 允許的滑動操作會重新計算 `圓餅圖區塊` (2.1)、`總餘額 / 控制區塊` (2.2) 以及 `逐筆紀錄區塊` (2.3) 的數據並觸發動畫。
...
- **3.3 報表設定 (Header 互動):**
    ...
    - **未來限制:** Header 中的日期/粒度控制不可跳到今天以後；若使用者嘗試選擇未來期間，需拒絕操作並提示原因。
