...
- **主內容區 (View):**
    ...
    - **結構:** 此區塊本身**不可**垂直滾動。它在內部垂直劃分為以下區域：
        ...
        - **2) 總餘額 / 控制區塊:** 位於圖表下方，顯示總餘額，並包含「新增支出」/「新增收入」按鈕，以及「列表分組切換」按鈕。
            
        - **3) 逐筆紀錄區塊 (FlatList):** 位於最下方，佔據剩餘高度，此區域**內部可垂直滾動**。
            
- **(已移除)** 核心操作 (FABs) 已合併到 `2.2 總餘額 / 控制區塊`。
...
- **2.2 總餘額 / 控制區塊:**
    ...
    - **內容 (總餘額):** 顯示當前時間區間內、所選帳戶的**總收入**、**總支出**、**總結餘** (均換算為基礎貨幣)。
        
    - **互動 (核心操作):** 包含**「新增支出」(-)** 和 **「新增收入」(+)** 按鈕。點擊導航至 `5.3`/`5.4`。
...
- **3.3 報表設定 (Header 互動):**
    ...
    - **預設行為:** App 啟動時，帳戶篩選器**預設僅選取** `IsPrimary` 為 `true` 的帳戶。
        
    - 使用者點擊 Header 中的「時間粒度篩選器」來切換時間粒度 (e.g., "Monthly")。    

- **3.4 導航邏輯 (Navigation Logic):**
    - 當使用者點擊「新增支出/收入」按鈕時：
        - **如果**當前 `HomeScreen` 的時間粒度為 `daily`，則導航至 `TransactionEditorScreen` 時，需傳入當前報表日期作為 `defaultDate` 參數。
        - **否則**，正常導航，不傳遞日期參數。
