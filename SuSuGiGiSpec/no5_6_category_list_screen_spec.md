...
- **2.2. 頁面主體 (Body):**
    - **2.2.1. 類別分區標題 (Category Section Headers):**
        - **UI:** 在每個類別列表（支出、收入）的頂部，應有一個標題列。此標題列包含：
            - **左側:** 區塊標題文字（例如：「支出」、「收入」）。
            - **右側:** 「新增」按鈕（例如：一個 `+` 圖示按鈕）。
        - **順序:** 「支出」區塊應顯示在「收入」區塊之上。
    - **2.2.2. 類別列表 (Category List):**
...
- **3.2. 互動邏輯:**
    ...
    - **點擊「新增」按鈕:**
        - 點擊「支出」區塊標題旁的「新增」按鈕後，導航至 `CategoryEditorScreen` (5.7) 的「新增」模式，並傳入 `categoryType` 為「支出」。
        - 點擊「收入」區塊標題旁的「新增」按鈕後，導航至 `CategoryEditorScreen` (5.7) 的「新增」模式，並傳入 `categoryType` 為「收入」。
...
- **退出:** 點擊頂部導航列的「返回」按鈕。
- **導出:**
    - 點擊列表項目或任一「新增」按鈕，導航至 `CategoryEditorScreen` (5.7)。
