...
此畫面應以 **Sheet 形式的 Modal** 呈現 (見 `no5_0` 全域設計原則)。

- **2.1. 頂部導航列 (Top Navigation Bar):**
    - **左側:** 「關閉」或「取消」按鈕。
    - **中間 (可點擊標題):**
        - **UI:** 顯示「支出」或「收入」，旁邊帶有向下小箭頭 (Chevron) 圖示。
        - **邏輯:** 在「新增」模式下可點擊切換，在「編輯」模式下不可點擊。
...
- **3.2. 儲存邏輯 (Save Logic):**
    - **新增模式:**
        - **(付費牆檢查)** 檢查目前使用者自訂類別數量是否已達免費版上限 (10個)。若已達上限，則導航至 `PaywallScreen` (5.16) 而非儲存。
