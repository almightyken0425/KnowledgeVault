...
- **報表顯示邏輯:**
    ...
    - **計算方式 (細化):**
        ...
        1. **確定篩選條件:** 根據使用者在首頁選擇的時間粒度、具體時間區間 (基於使用者設定時區計算 UTC 範圍)，以及納入統計的帳戶列表 (由 `no5_2_home_screen_spec.md` 中的「帳戶篩選器」決定)。
...
> **PM 註記：匯率設計原則**
> 
> 1.  **(a) 報表計算分離:** 報表在計算總資產時，`Transfers` 的價值流動是基於 `AmountFromCents`/`AmountToCents` 直接計算的；`CurrencyRates` 表中的匯率主要用於換算**非基礎貨幣**的 `Transactions` (收支)。
> 2.  **(b) 自動記錄 (私下匯率):** 「跨幣別轉帳」會自動將該筆交易的隱含匯率新增至 `CurrencyRates` 表。這能即時更新匯率，但也可能記錄到使用者「私下換匯」的特殊匯率。
> 3.  **(c) 手動覆蓋 (市場匯率):** 因此，系統仍須提供手動管理匯率的功能 (`no5_11`, `no5_12`)，允許使用者隨時輸入一筆新的「市場匯率」。這筆新匯率將會成為最新的有效匯率，用以覆蓋舊的匯率，確保未來的 `Transactions` 換算能採用更準確的市場價值。
> 4.  **(d) 不串接 API:** 基於 MVP 的成本、效能考量，以及 App 不回溯歷史資產價值的定位（報表一律採用最新的匯率），決定不串接第三方匯率 API。
...
- **首次登入流程:**
    ...
    4. **若無資料 (新用戶):**
        ...
        - 建立一個預設帳戶 (例如「現金」)，並設定其 `SortOrder = 1`。此帳戶將作為新增交易時的預設選項。
            
        - 建立預設的 `Categories` (共 5 個，讓免費版使用者有新增空間):
                - **支出 (4):** 餐飲、交通、購物、帳單。
                - **收入 (1):** 薪水。
...
### 7.4. 快速捕捉小工具 (Widgets) - _[延後功能]_

- **說明:** 此功能非 MVP 範疇，詳細規格已移至 `no3_future_consideration.md`。
...
- **付費版 (Premium Tier):**
    ...
    - **進階報表與資料:**
        ...
        - 匯出資料 (`.csv`, `.db`) 功能 (2.6) — Premium 專屬且延後至付費版本釋出。
