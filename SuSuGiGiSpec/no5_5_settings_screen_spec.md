    ...
    - **項目:**
        - **第一組：資料管理 (Data Management)**
            - **類別管理:**
                - **入口 UI:** 包含圖示和「類別管理」文字。
                - **流程:**
                    1.  **導航至 `CategoryListScreen` (5.6):**
                        - **UI:** 依 `CategoryType` (收/支) 分頁顯示類別列表、新增按鈕 (FAB)。
                        - **互動:** 支援拖拉排序，更新 `SortOrder` 欄位。
                    2.  **導航至 `CategoryEditorScreen` (5.7):**
            ...
            - **帳戶管理:**
                - **入口 UI:** 包含圖示和「帳戶管理」文字。
                - **流程:**
                    1.  **導航至 `AccountListScreen` (5.8):**
                        - **UI:** 顯示所有帳戶列表，並可拖拉排序。
                        - **互動:** 支援拖拉排序。
                    2.  **導航至 `AccountEditorScreen` (5.9):**
                        - **觸發:** 點擊列表中的現有帳戶，或點擊「新增」按鈕。
                        - **UI:** 提供名稱、圖示、幣別、初始餘額等欄位。
                        - **邏輯:** 處理新增/編輯/刪除，並檢查免費版 3 個帳戶的限制。
            - **匯率管理:**
                - **UI:** 包含圖示和「匯率管理」文字，並在右側顯示一個 "Premium" 標籤。
                - **導航:** 點擊後導航至 `CurrencyRateScreen` (5.11)，用於管理各貨幣對的最新有效匯率。

        - **第二組：偏好設定 (Preferences)**
            - **偏好設定:**
                - **UI:** 包含圖示和「偏好設定」文字。
                - **導航:** 點擊後導航至 `PreferenceScreen` (5.13)，用於設定基礎貨幣、時區、語系，並提供清除資料等進階選項。

        - **第三組：升級 (Upgrade)**
            - **升級至 Premium:**
                - **UI:** 一個醒目的項目，包含圖示和「升級至 Premium」文字。
                - **邏輯:** 此項目**僅對免費版使用者顯示**。
                - **導航:** 點擊後導航至 `PaywallScreen` (5.16)。
