# 5.11. 設定 - 匯率管理 (CurrencyRateListScreen)
...
- **2.4. 新增按鈕 (Add Button):**
    - **UI:** 一個懸浮操作按鈕 (FAB)，圖示為「+」。
    - **邏輯:** 點擊後導航至 `CurrencyRateEditorScreen` (5.12) 的「新增」模式。
...
- **3.2. 互動邏輯:**
    - **點擊列表項目 (設定新匯率):** 點擊列表項目應被視為「更新匯率」。此操作會導航至 `CurrencyRateEditorScreen` (5.12)，並傳入該貨幣對資訊以「新增」一筆新的匯率記錄。
    - **設定新匯率的行為:** 在 `CurrencyRateEditorScreen` 中儲存後，程式將遵循「只增不改」原則，在 `CurrencyRates` 表中**新增一筆記錄**，使其成為該貨幣對新的「最新匯率」。
...
- **退出:** 點擊頂部導航列的「返回」按鈕。
- **導出:** 點擊列表項目或新增按鈕，導航至 `CurrencyRateEditorScreen` (5.12)。
