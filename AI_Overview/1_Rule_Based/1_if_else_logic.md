...
### 優點 Pros
- **可解釋性 Explainability:** 100% 透明。銀行審核貸款如果拒絕你，Rule-Based 系統可以精確告訴你是因為 年收入小於 50 萬。
- **絕對控制:** 不會有 AI 幻覺 或種族歧視問題，除非規則本身就是歧視的。
- **效能極致:** 運算極快，通常只是幾個 CPU 指令，不需要 GPU。

### 缺點 Cons
- **維護災難:** 當變數從 2 個變成 2000 個，If-Else 會變成無法維護的 義大利麵條代碼 Spaghetti Code。
- **無法處理感知問題:** 你無法寫出一組 If-Else 規則來定義 什麼是貓，例如貓有耳朵？狗也有。貓有鬍鬚？老鼠也有。
- **零適應性:** 環境一變，例如溫度單位從攝氏變華氏，規則全毀。
