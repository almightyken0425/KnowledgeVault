...
1.  **訓練穩定:** 沒有警察與小偷的勾心鬥角，目標函數非常單純 (就是預測雜訊)，所以很難訓練失敗。
2.  **多樣性:** 不會發生 **模式崩潰**，因為每次從不同的隨機雜訊開始，去噪出來的結果都會不一樣。
3.  **可控性:** 我們可以透過 **文字引導 Text Guidance** (CLIP) 來控制去噪的方向，告訴它：「請往『一隻在太空中的貓』的方向去噪。」

## 代價

天下沒有免費的午餐。GAN 生成圖片只需要跑一次神經網路，速度極快 (毫秒級)。
Diffusion Model 生成圖片需要跑幾十次甚至上百次去噪步驟，速度較慢 (秒級甚至分鐘級)。但這換來的是無與倫比的畫質與細節。
