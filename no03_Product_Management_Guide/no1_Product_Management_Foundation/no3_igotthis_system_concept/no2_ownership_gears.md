...
- 其餘屬性皆由此開關推導，不是各領域自填
- 推導出的屬性：有無階層、可否被錨定、匯出粒度、merge 的有無與執行者
...
| 階層 | 不成立 | 不成立，分層是使用者約定 |
| 錨點標的 | 不可被錨定 | roadmap repo 的 commit |
| merge 時機與執行者 | 無 merge 事件，寫入即生效 | 人在 git 執行，時機是領域約定 |
| git 語意 | 無 git | 真相所在 |
| 追溯精度 | 資料列，稽核可到欄位級 | commit，不到條目 |
| 匯出粒度 | 資料列，以匯出功能補可攜性 | 只給 repo 路徑與 commit |

- 有 git、誰能編輯、錨點標的、階層皆由檔位推導，領域屬性矩陣不逐項列出
- 有無階層：兩檔皆不成立
  - 分層屬掛勾 repo 內的資料，系統管不到，故矩陣不列
- 錨點標的：檔位 A 不可被錨定，檔位 C 是 roadmap repo 的 commit
  - 錨點標的僅 roadmap 域實例化，其餘檔位 C 域的 commit 走一般 commit 指標欄位
