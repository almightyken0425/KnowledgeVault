...
```mermaid
flowchart LR
    A["需求段<br/>任何角色開單提出修改需求"] --> B["決策段<br/>roadmap task merge 即定案"]
    B --> C["執行段<br/>Epic 寫明需求來源後施工"]
...
- bug 急修走 severity 分流：
  - P0 與 P1 走快速通道：直接開 Epic，需求來源寫明急修對象，事後補 roadmap task
...
- 快速通道的補單：
  - 補單規則的機制掛點待定，見風險與待決篇
  - 急修速度不受影響
- PM 開 roadmap task 並在 roadmap repo 開 branch，關連對應的 .item
- 基數規則：
  - 一張 roadmap task 可關連多個 .item
  - 一個 .item 只能關連一張 roadmap task
  - 發現需要連到多張 → 拆 .item
...
- 一張 roadmap task 對應 roadmap repo 的一條 branch
...
- 開 Epic 時寫明需求來源與要產出什麼
- 需求來源留白即例行工作，如發版回歸、例行維護
- roadmap main 前進時的對照由 PM 人工進行，系統不提示
...
- 推翻本身是新 requirement → 新 roadmap task revert 決策
