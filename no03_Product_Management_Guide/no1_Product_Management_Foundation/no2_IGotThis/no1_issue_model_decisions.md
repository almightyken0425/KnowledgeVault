...
> 本篇記錄 IGotThis 工單資料模型的定案，含容器骨架、關聯機制、檢視層與欄位總表。內容由系統結構圖逐題推導而來，每項定案附選擇理由。尚未定案的項目集中收在末段。
...
- 本篇範圍內的項目皆已定案
- 部分定案以本篇範圍外的工作為前提，列出依賴以免遺漏
- **權限模型未設計，兩處定案以它為前提:**
    - 跨團隊檢視同一產品，由權限解決而非由結構解決
    - 多筆欄位不另設權限，可見範圍等同所屬工單
- **工作日與假日未設定，一處定案受影響:**
    - 偏離天數的計算基準是否扣除假日未定
- **檢視層僅盤點三種形態:**
    - 已盤開發順序表、看板、燃盡圖，欄位依賴皆可滿足
    - 其餘形態未盤，可能帶出新的必帶欄位
