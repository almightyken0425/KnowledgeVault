...
- **資料溯源追蹤:** 每個欄位、每個數值都追溯到源頭，說明「從哪裡來、經過什麼運算、填到哪裡去」
  - **層層拆解範例:** 類似物理學介紹功率時不只說「牛頓/秒」，還要拆解成「(m×a)/秒 = (kg×m/s²)/秒 = kg×m/s³」
...
- **技巧:**
  ...
  - **從整體到部分:**
    - 範例：先介紹 Block Header + Block Body，再展開 Header 的 6 個欄位
...
- **技巧:**
  - **先「是什麼」再「為什麼」:**
    ...
    - 目的：讓讀者先建立事實認知，再加深理解
  - **分階段揭露:**
    - 範例：交易建立文件分為 Payload 定義 → 欄位溯源 → API 參數生成
    - 目的：每次只讓讀者處理一個層次的資訊
  - **延遲細節:**
...
- **技巧:**
  ...
  - **視覺化範例:**
    ...
    - 目的：讀者可以對照自己的理解
  - **事實 + 範例 + 結論:**
    - 範例：「你有 10oz 金幣 → 支付 3oz → 鑄造 3oz+7oz」
    - 目的：三段式說明讓讀者自我驗證理解
...
- **技巧:**
  - **文字 + 代碼 + 視覺:**
    - 範例：Merkle Root 用文字描述 + Python 代碼 + ASCII 樹狀圖
    - 目的：視覺型與語言型學習者都能理解
  - **抽象 + 具體:**
    - 範例：「工作量證明」 + 「像擲 2^256 面的骰子」
    - 目的：抽象概念用具體比喻強化
  - **中文 + 英文:**
...
- **技巧:**
  - **正常流程 + 邊緣案例:**
    ...
    - 目的：確保讀者理解機制的邊界
  - **成功案例 + 失敗案例:**
    - 範例：「成功 Hash: 小於 Target ✓」 + 「失敗 Hash: 大於 Target ✗」
    - 目的：透過對比加深印象
  - **IF 條件分支:**
    - 範例：「IF 區塊 A' 成為孤兒塊: 獎勵消失」
    - 目的：讓讀者模擬不同場景的結果
...
- **技巧:**
  - **因果鏈:**
    - 範例：「修改歷史區塊 → Hash 改變 → 後續區塊全部無效」
    - 目的：讓讀者理解連鎖效應
  - **經濟後果:**
    - 範例：「硬分叉 → 永久分裂 → 生態擕裂 → 市值波動」
    - 目的：建立完整的後果心智模型
...
- **協議層 Protocol Layer:**
  - 描述「規則是什麼」
  - 範例：創世協議規定 Public Key Hash 必須經過 SHA256 + RIPEMD160
...
- **協議視角 Protocol View:**
  - 描述規則本身如何定義，不涉及實作細節
  - 範例：「協議規定 Public Key Hash 必須經過 SHA256 + RIPEMD160」
- **節點視角 Node View:**
  - 描述節點如何執行驗證與處理邏輯
  - 範例：「節點收到新區塊後的 SOP：驗證 PoW、驗證交易、更新帳本」
- **使用者視角 User View:**
  - 描述錢包或客戶端如何與系統互動
...
繼承 `DocWritingPolicy.md` 的所有禁止項目，並額外增加：
...
除了 `DocWritingPolicy.md` 定義的關鍵字外，技術文件增加以下關鍵字：
...
### 中英對照
- **首次出現:** 中文名稱 英文全稱
- **範例:** 私鑰 Private Key、工作量證明 Proof of Work
- **後續使用:** 優先使用中文，必要時加註英文縮寫如 PoW

### 數學公式
- **使用 LaTeX 語法:** 用 `$` 包裹行內公式，`$$` 包裹區塊公式
- **範例:** 
  - 行內：私鑰範圍為 $1$ 到 $2^{256}-1$
  - 區塊：
    ```
    $$
    NewTarget = OldTarget \times \frac{ActualTime}{TargetTime}
    $$
    ```

### 二進制資料
- **十六進制表示:** 使用 `0x` 前綴或直接 Hex String
- **範例:** `0x00000000FFFF0000...` 或 `ea34567...`

### 資料流圖
- **使用 Mermaid 圖表:** 描述複雜的資料流或狀態轉換
- **範例:**
  ```mermaid
  graph LR
      A[Wallet] -->|Sign Transaction| B[Mempool]
      B -->|Select Tx| C[Miner]
      C -->|Broadcast Block| D[Node]
      D -->|Verify & Sync| E[Blockchain]
  ```
...
### 引用原則
- **避免硬編碼檔名:** 不直接寫 `依據 no1_1_genesis_protocol_code.md`
- **使用主題引用:** 改為 `依據創世協議規範` 或 `參見身份系統定義`
- **層級依賴:** 高層文件可引用低層概念，反之則不行

### 術語一致性
- **建立術語表:** 統一關鍵術語的中英文對照
- **範例:**
  - Private Key = 私鑰
  - Public Key Hash = 公鑰雜湊
  - UTXO = 未花費交易輸出
...
- **協議規範層:** `no1_1_genesis_protocol_code.md`
  - 展示如何定義核心協議與不可變規則
- **資料結構層:** `no2_2_block_and_chain.md`
  - 展示如何用金幣譬喻說明 UTXO 模型
- **軟體實作層:** `no3_1_node_and_wallet.md`
  - 展示如何區分協議定義與軟體角色
- **運作流程層:** `no4_1_transaction_creation.md`
  - 展示如何追溯每個欄位的來源與生成
- **共識治理層:** `no5_2_fork_types.md`
  - 展示如何對比不同機制並提供實際案例
