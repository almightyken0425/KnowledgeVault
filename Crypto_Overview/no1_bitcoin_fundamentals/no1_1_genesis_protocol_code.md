...
```json
{
  ...
  "block_header": {
    "version": 1,                      // 版本號: 用於追蹤軟體或協議的升級
    "prev_block_hash": "0000...abc",   // 前一個區塊的 Hash 值: 確保區塊鏈的連續性與不可篡改性
    "merkle_root": "ea34...567",       // Merkle Tree 根雜湊值: 該區塊內所有交易的指紋,用於快速驗證交易存在性
    "timestamp": 1234567890,           // 時間戳記: 區塊產生的時間
    "bits": 486604799,                 // 難度目標值: 決定該區塊 Hash 必須小於多少才能被網路接受
    "nonce": 2083236893                // 隨機數: 礦工不斷調整此數值以尋找符合難度目標的 Hash
  },
  // 區塊體: 實際存放交易資料的容器
  "tx_count": 2,                       // 交易總數: 該區塊內包含的交易總數量
  "transactions": [
    {
      "type": "Coinbase",              // 第一筆交易: 用於發放區塊獎勵給礦工
      // ... 交易詳細內容
    },
    {
      "type": "Normal",                // 其餘的標準交易列表
      // ... 交易詳細內容
    }
  ]
}
