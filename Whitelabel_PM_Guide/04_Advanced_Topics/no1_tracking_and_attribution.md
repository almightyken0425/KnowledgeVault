# 廣告追蹤與歸因：Meta Pixel 和 Click ID 的完整流程

## 角色定位

廣告追蹤是站方評估廣告效益的基礎工具，沒有正確的追蹤設定，廣告費等同在黑暗中燒，PM 需要理解追蹤碼的工作原理，才能設計出站方真正需要的後台設定功能，以及確認串接是否正確。
...
**業界建議做法**：兩者並行使用，Server-Side 為主，Client-Side 補充，利用 eventID 去重，避免同一個事件被計算兩次。
...
方案一適合大多數事件要對所有 Pixel 觸發的情境，方案二適合需要精確控制某個事件只發給特定廣告商的情境。
